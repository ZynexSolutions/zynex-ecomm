import React, { useRef, useEffect, useState, useCallback, useMemo } from 'react'; // Import useMemo
...
const CustomRangeSlider = React.memo(({ // <-- Use React.memo
    min = 0,
    max = 100,
    step = 1,
    value,
    ...
    const calculateValueFromPosition = useCallback((clientX) => {
        ...
        let relativeX = Math.max(0, Math.min(trackWidth, clientX - trackRect.left));
    ...
    const handleInteractionMove = useCallback((e) => {
        ...
        e.preventDefault();
    }, [isDragging, calculateValueFromPosition, value, throttledOnChange]); // Add throttledOnChange to deps
    ...
    return (
        <div
            ref={sliderRef}
            // ... (rest of the JSX)
            className={`custom-slider-container ${className} ${isDragging ? 'dragging' : ''}`}
